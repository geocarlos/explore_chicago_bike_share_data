...
male = len([x for x in data_list if x[-2] == 'Male'])
female = len([x for x in data_list if x[-2] == 'Female'])
...
"""
def most_popular_gender(data_list):
    dl = count_gender(data_list)
    answer = "Equal" if dl[0] == dl[1] else ("Male" if dl[0] > dl[1] else "Female")
    return answer
...
u_types = ["Customer", "Subscriber", "Dependent"]
quantity = [
        len([x for x in data_list if x[-3] == "Customer"]),
        len([x for x in data_list if x[-3] == "Subscriber"]),
        len([x for x in data_list if x[-3] == "Dependent"])
    ]
y_pos = list(range(len(u_types)))
plt.bar(y_pos, quantity)
plt.ylabel('Quantity')
plt.xlabel('User Type')
plt.xticks(y_pos, u_types)
...
"""
def get_median(number_list):
    l = sorted([float(x) for x in number_list])
    if len(l) % 2 == 0:
...
user_types = set([s[3] for s in data_list])
...
"""
def count_items(column_list):
    item_types = list(set([x for x in column_list]))
    count_items = []
    for t in item_types:
        count_items.append(len([x for x in column_list if x == t]))
